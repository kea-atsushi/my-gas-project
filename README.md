...
Merchantの商品同期・承認状態、Search ConsoleのSEO、GoogleビジネスプロフィールのMEOも既存の日次処理内で監視します。単なる件数変化ではなく原因を判定し、原則として「要対応」だけを通知します。「要確認」は新規・悪化分だけ日次ダイジェストへまとめ、「対応不要」だけの日はメールを送りません。
...
- Shopify・Google Adsなどの取得失敗は0へ変換しません。取得時刻、API応答概要、エラー、前回成功値を保持し、成功応答後の実績0と明確に分けます。
- Recommendationsは同一原因・同一対象を重複登録せず、原因、対象、根拠、変更内容、期待効果、リスクを記録します。未検証の一括変更は作りません。

## 通知判定

- `対応不要`: 正常取得後の売上・注文・広告費0、Merchantの商品総数と承認数だけが同数減る小規模な削除・非公開パターン、不承認の減少など。記録のみで通知しません。
- `要確認`: 初回のAPI取得失敗、審査中増加など原因未確定の変化、十分な表示・順位・商業意図があるSEO候補。新規分だけ日次ダイジェストへまとめます。
- `要対応`: 2回連続のAPI取得失敗、新しいMerchant不承認、Shopify注文があるのにGoogle Ads購入CVが0、主要URLの新しい重大問題など。原因・対象・根拠・次の操作を付けて通知します。

Merchant不承認は商品別の実issueを取得し、価格、在庫・販売可否、画像、GTIN・商品識別子、ブランド、MPN、配送・送料、ポリシー、その他へ分類します。通知は新しく増えた商品issueだけ、Recommendationsは同一原因ごとに1件へ集約します。

SEO候補は表示100回未満、URL・ドメイン形式、絵文字、長い英数字列などのノイズを除外します。残った候補もCTR、順位、表示回数、商品・コレクションページ、購入意図語をスコア化し、ページ単位で上位5件までにまとめます。
...
## ブランド名単体TOP10：8/16の週次判定

2026-09-24に確認した16ブランドを固定分母にする。在庫・商品掲載の増減で分母を変えない。既存定義（英字・日本語等の単体語で対象コレクションの平均順位が最良の表記、同順位は表示数順）を継続し、1〜2表示のTOP10は暫定、GSC無観測はunknownと表示する。8週目の2026-11-19に8/16到達を狙い、12週目の12-17までに安定させる。安定性は複数週の実績と表示数を見て判断し、一度の少量TOP10で達成としない。

既存6時の日次順位取得が成功した後、既存 `BrandSEOActionPlan` に月曜始まりで週1回、16ブランドの判定を追記する。月曜失敗時は次の正常取得で補完する。旧17列と既存行を保持し、同週の再実行は追記しない。48時間以内の完全な観測がそろわない場合、過去判定を維持して明示エラーにする。新規シート・トリガー・メール送信は増やさない。既存ダッシュボード・週次提案に現在の集中対象と次の対応を表示する。

初回集中対象はButton Works、Oblada、SEA、COEL、77circa。Aは平均順位10超〜20、Bは20超〜40、Cは40超または未観測。技術異常を優先し、A→B→Cの順に5枠を選ぶ。Cは公開商品数を補助判断とし、世界的大手との競合が強いブランドは後順位にする。公開商品数は検索需要の推定ではない。TOP10入りは維持へ移し次候補を繰り上げる。

週次履歴には同一語・同一URLの直近7日／前7日順位、表示、クリック、CTR、TOP10、前週改善幅、今週の変更、次の対応を記録する。集中14日で再評価を促し、21日で十分な表示数を伴う1位以上の改善を確認できなければ再評価へ移し、14日間は他候補を優先する。無観測・少量は効果なしと断定せず検索意図・競合・本文・内部リンク・canonical・index・外部評価を確認する。自動判定はSEO変更や外部連絡を実行しない。
